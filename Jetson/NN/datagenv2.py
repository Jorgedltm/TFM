...
class DataGenerator(Sequence):
    ...
    def __init__(self, dataset, batch_size=32, shuffle=True, characteristics=False,
    ...
    def __getitem__(self, idx):
        ...
        if self.characteristics:
            ...
            return np.array(spectrogram_in).astype('float32'), np.array(embedding).astype(dtype), \
...
if __name__ == '__main__':
    ...
    dataset = EmbedGen('../../pyrirtool/recorded_RIR', 'room_impulse', debugging=False, normalization=True, normalize_vector=True)
    test_rirs = DataGenerator(dataset, batch_size=3, shuffle=True)
