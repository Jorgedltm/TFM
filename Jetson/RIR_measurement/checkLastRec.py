...
RIR_direc = 'newrir_mic[77, 223, 92]_spk[195, 270, 45]'
RIR = np.load(f'recorded/{RIR_direc}/RIR.npy')
maxval = np.max(RIR)
minval = np.min(RIR)
...
for idx in range(numplots):
    ...
    plt.savefig(f"Figures/{RIR_direc}RecSignal.png")
...
plt.savefig(f"Figures/{RIR_direc}Spec.png")
